import { Component, OnInit } from '@angular/core';
import { MovieService } from 'src/app/apiService/movie.service';
import { Movie } from 'src/app/movie';
import { environment } from 'src/environments/environment';

@Component({
  selector: 'app-random',
  templateUrl: './random.component.html',
  styleUrls: ['./random.component.css']
})

export class RandomComponent implements OnInit {

  constructor(private movie: MovieService) { }

  ngOnInit(): void {
    this.InitMaxMovies();
    this.InitGenres();
  }

  randomResult: any;
  latest: any;
  genresFR: any;

  InitMaxMovies() {
    this.movie.GetLatest().subscribe(
      {
        next: (data:any) => this.latest = data,
        error: (e) => console.log("Erreur dans InitMaxMovies")
      }
    );
  }

  InitGenres() {
    this.movie.GetGenres().subscribe(
      {
        next: (data:any) => this.genresFR = data,
        error: (e) => console.log("Erreur dans InitGenres")
      }
    )
  }
  /**
   * Traduction des genres par leur id
   */
  private translateGenres() {
    const idEN = this.randomResult.genres.map( (e:any) => e.id );
    const idFR = this.genresFR.genres.map( (e:any) => e.id );
    idEN.forEach( (element:string, index:number) => {
      let position = idFR.indexOf(element);
      if( position > -1 ) {
        this.randomResult.genres[index].name = this.genresFR.genres[position].name;
      }
    });
  }

  /**
   * Image générique si pas de jaquette, et met à jour le chemin pour qu'il fonctionne
   */
  private updatePoster() {
    if(this.randomResult.poster_path) {
      this.randomResult.poster_path = environment.imageUrl + "w500" + this.randomResult.poster_path;
    }
    else
    {
      this.randomResult.poster_path = "data:image/png;base64,";
    }
  }
  /**
   * Evite les codes html, par exemple dans movie.description
   * @param input le texte à décoder (ex, remplace &amp; par &)
   * @returns le texte en clair
   */
  private htmlDecode(input:string) {
    var doc = new DOMParser().parseFromString(input, "text/html");
    if(doc.documentElement.textContent) {
      console.log("decode");
      return doc.documentElement.textContent;
    } else {
      return input;
    }
  }
  /**
   * Renvoie un film au hasard dans les 20 plus tendances
   */
  RandomTendance(): any {
    this.movie.GetPopulars().subscribe({
      next: (data:any) => {
        let randomPopulars = data;
        let max = randomPopulars.results.length;
        let rnd = Math.floor(Math.random() * max); //tableau de 0 à max
        //console.log(randomPopulars.results[rnd]);
        this.randomResult = randomPopulars.results[rnd]; //on a les principaux détails en retour FR
        this.updatePoster();
        //On récupère les détails complets
        let id = randomPopulars.results[rnd].id; //lit l'id du film tiré au sort
        this.movie.FindMovieById(id).subscribe({
          next: (film:any) => {
            //console.log(film);
            this.randomResult = film;
            this.randomResult.title = randomPopulars.results[rnd].title;
            this.randomResult.overview = this.htmlDecode(randomPopulars.results[rnd].overview);
            const d = new Date(Date.parse(this.randomResult.release_date));
            this.randomResult.release_date = d.toLocaleDateString();
            this.updatePoster();
            this.translateGenres();
          },
          error: (e) => {
            console.log("Erreur lecture film id " + id);
          }
        })
      },
      error: (e) => {
        console.log("Une erreur fatale s'est produite...");
      }
    })
  }

  /**
   * Renvoie un film par son id au hasard (mais pas un film adulte)
   * @param max le nombre maximum de films dans la base
   * @param retries plusieurs essais possibles, car des id peuvent avoir été supprimés
   */
  RandomMovie(max: number, retries?: number): any {
    let id = Math.floor(Math.random()*max)+1;
    this.movie.FindMovie(id).subscribe({
      next: (data:any) => {
        if(!data.adult) {
          this.randomResult = data;
          this.updatePoster();
          this.translateGenres();
        } else {
          console.log("oups film pour adultes... on recommence");
          this.RandomMovie(max);
        }
      },
      error: (e) => {
        console.log('HTTP Error '+retries);
        if( retries == undefined ) retries = 0;
        if( ++retries < 5 ) { this.RandomMovie(max, retries) }
      }
      });
  }

}
